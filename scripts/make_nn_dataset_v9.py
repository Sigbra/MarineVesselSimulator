...
# -*- coding: utf-8 -*-
"""
make_nn_dataset_v9.py  (window-aware, time-series safe, with TEST split)

Builds train/val/test CSVs + normalization stats for velocity-only observer (v9).

Inputs (10): ax, ay, az, qw, qx, qy, qz, tau_x, tau_y, tau_n
  - ax..az come from SIMDATA cols 46..48 (imu.accel)
  - qw..qz come from SIMDATA cols 52..55 (q_nb: Body→END, custom END convention)
  - tau_x,tau_y,tau_n come from SIMDATA cols 27..29 (tau_XYN actual, BODY @ CO) by default
    (use --tau_source commanded to take cols 24..26 instead)

Aux (3): w_est_x, w_est_y, w_est_z
  - from SIMDATA cols 56..58 (BODY angular rates estimated by q-observer)

Outputs (3): vE, vN, vD
  - computed via v^n = R_nb(q_nb) · [u,v,w]^b  where u,v,w are SIMDATA cols 1..3
...
- NEW: separate --val_frac and --test_frac (train = 1 - val - test).
...
def windows(df: pd.DataFrame, seq: int) -> List[pd.DataFrame]:
    ...
    if N % seq != 0:
        print(f"[make_nn_dataset_v9] Dropping {N - B*seq} tail rows to fit {B} full windows of {seq}.")
...
def main():
    ap = argparse.ArgumentParser(description="Make v9 dataset (IMU accel + quaternion + BODY τ_XYN -> END velocity) + aux w_est.")
    ...
    if not args.shuffle_windows or args.seq <= 0:
        ...
        # Window-aware shuffle then split by windows
    ...
    # Persist split summary for reproducibility
    ...
    print(f"Wrote: {test_csv}   ({len(test_df)} rows)")
    print(f"Wrote: {os.path.join(args.out_dir, 'norm_stats.json')}")
    print(f"Wrote: {os.path.join(args.out_dir, 'split_summary.json')}")
    if args.shuffle_windows and args.seq > 0:
        print(f"Windowed with seq={args.seq}; files are concatenations of intact {args.seq}-step windows (shuffled across windows only).")
    else:
        print("Chronological split by rows (no window shuffling).")
    print(f"τ source: {args.tau_source}  (cols {tau_cols})")
...
# # -*- coding: utf-8 -*-
# """
# make_nn_dataset_v9.py  (window-aware, time-series safe)

# Builds train/val CSVs + normalization stats for velocity-only observer (v9).
...
#     ap.add_argument("--tau_source", choices=["actual","commanded"], default="actual",
#                     help="Use tau_XYN actual (27..29) or commanded (24..26) as inputs.")
#     args = argparse.ArgumentParser.parse_args(ap)
