#!/usr/bin/env python3
"""
Create a tidy NN training/validation dataset (v5) from simulated boat logs.
...
Heading handling:
- NN inputs use **(cpsi, spsi)** from the **previous-timestep** heading per run_id.
- We also add **current** heading as targets **(cpsi_t, spsi_t)** (no raw psi column in outputs).
...
- v_nav = Rzyx(phi, theta, psi) * v_body, rows [East; North; Down]:
...
def _compute_nav_vel(u: pd.Series, v: pd.Series, w: pd.Series,
    ...
    """Compute navigation-frame horizontal velocities (Vn, Ve) from body-frame velocities
    using the provided Rzyx (rows East, North, Down). Returns (Vn, Ve)."""
...
def extract_df(df: pd.DataFrame) -> pd.DataFrame:
    """Extract required columns and compute Vn, Ve. Keep psi internally; convert later to cos/sin."""
    ...
    psi   = _x_at_block(df, X_INDEX["psi"]).astype(float)  # current yaw (internal)

    Vn, Ve = _compute_nav_vel(u, v, w, phi, theta, psi)
    ...
    out = pd.DataFrame({
        ...
        # keep psi temporarily; we will produce inputs/targets from it per run
        "psi":  psi,
        # targets
        "Vn": Vn, "Ve": Ve,
        "p": p, "q": q, "r": r,
...
def add_prev_cpsi_spsi_and_targets(df: pd.DataFrame) -> pd.DataFrame:
    """
    Within each run_id segment:
      inputs : cpsi = cos(psi_{t-1}), spsi = sin(psi_{t-1})
      targets: cpsi_t = cos(psi_t),    spsi_t = sin(psi_t)
    Drops the first row (no previous).
    Removes 'psi' afterwards.
    """
    def _per_run(g: pd.DataFrame) -> pd.DataFrame:
        g = g.copy()
        psi_prev = g["psi"].shift(1)
        g["cpsi"] = np.cos(psi_prev)
        g["spsi"] = np.sin(psi_prev)
        g["cpsi_t"] = np.cos(g["psi"])
        g["spsi_t"] = np.sin(g["psi"])
        g = g.dropna(subset=["cpsi","spsi"]).reset_index(drop=True)
        return g.drop(columns=["psi"])
...
def main():
    ...
    ap.add_argument("--out_dir", default=None, help="Output folder; default = <repo>/data/nn_dataset_v5")
    ...
    default_out_dir  = default_data_dir / "nn_dataset_v5"
    ...
    # File-level split if multiple files; otherwise intra-split
    if len(files) > 1:
    ...
    for f in files:
        ...
        if goes_to_val:
            dval = df_run.copy()
            dval.insert(0, "run_id", base_id)
            frames_val.append(dval)
            ...
            if len(files) == 1 or len(val_files) == 0:
                ...
                dtr = df_run.copy()
                dtr.insert(0, "run_id", base_id)
                frames_train.append(dtr)
    ...
    cols_out = [
        ...
        "Vn","Ve","p","q","r","phi","theta","cpsi_t","spsi_t"
    ]
    ...
    # Build inputs/targets per run: prev (cpsi,spsi), current (cpsi_t,spsi_t); drop first row and remove 'psi'
    if len(df_train): df_train = add_prev_cpsi_spsi_and_targets(df_train)
    if len(df_val):   df_val   = add_prev_cpsi_spsi_and_targets(df_val)
    ...
    if len(df_val) == 0 and len(df_train) > 0:
        ...
        tr = add_prev_cpsi_spsi_and_targets(tr)
        va = add_prev_cpsi_spsi_and_targets(va)
    ...
    # NOTE: psi itself is not part of inputs; only (cpsi,spsi) are fed to the NN.
    input_cols  = ["ax","ay","az","wx","wy","wz","cpsi","spsi"]
    target_cols = ["Vn","Ve","p","q","r","phi","theta","cpsi_t","spsi_t"]
