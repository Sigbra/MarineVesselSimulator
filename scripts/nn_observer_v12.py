...
python3 -u scripts/nn_observer_v12.py \
  --train data/nn_dataset_v12_final2/train.csv \
  --val   data/nn_dataset_v12_final2/val.csv \
  --test  data/nn_dataset_v12_final2/test.csv \
  --out   data/nn_model_v12_final2 \
  --epochs 500 --gpu --ensemble 8 \
  --norm_json data/nn_dataset_v12_final2/norm_stats.json \
...
def read_csv_columns(path: str,
    ...
    # Wrap Euler angles to ±180° (in radians: [-pi, +pi))
...
class VelNetV12(nn.Module):
    ...
    forward accepts [B,T,9] or [B,1,9]  → returns (y:[B,T,3], h_next)
    """
    def __init__(self,
                 input_dim: int = 9,
                 hidden: int = 128,
...
class TrainCfg:
    ...
    print_period: int = 5
    tbptt: int = 256            # unused in chunked path; kept for parity
...
class StatefulOneStep(torch.nn.Module):
    ...
    Methods:
      - forward(x:[1,1,9], h:[L,1,H]) -> (y:[1,1,3], h_next)
