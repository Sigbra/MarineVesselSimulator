...
# -*- coding: utf-8 -*-
"""
nn_observer_v10.py (Same as v9 but tuned for real data)
...
Run example:
python3 scripts/nn_observer_v10.py \
--train data/nn_dataset_v9_real/train.csv \
--val   data/nn_dataset_v9_real/val.csv \
--test  data/nn_dataset_v9_real/test.csv \
--out   data/nn_model_v10_ens4_real \
--seq 200 \
--epochs 10000 \
--gpu \
--ensemble 3 \
--norm_json data/nn_dataset_v9_real/norm_stats.json
...
def main():
    ...
    for m in range(args.ensemble):
        ...
        model = VelNetv10(hidden=args.qwidth, num_layers=3, dropout_p=args.dropout).to(device)
    ...
    for pth in member_ckpts:
        if not os.path.isfile(pth):
            ...
            continue
        m = VelNetv10(hidden=args.qwidth, num_layers=3, dropout_p=args.dropout).eval().to("cpu")
        ckpt = torch.load(pth, map_location="cpu")
        state = ckpt.get("state_dict", ckpt)
        m.load_state_dict(state, strict=True)
        members.append(m)
