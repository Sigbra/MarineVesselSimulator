# scripts/nn_observer_v4.py
# -------------------------------------------------------
# GRU observer (v4) with ENSEMBLE predictive covariance for EKF.
#
# Inputs (8D): 3D accel (ax, ay, az), 3D gyro (wx, wy, wz), and heading encoded
#              as (cpsi, spsi) = (cos(psi), sin(psi))
# Targets (7D): [Vn, Ve, p, q, r, phi, theta]
#
# Normalization (train-split stats):
#   - Dataset gives x̂ = (x-μ_in)/σ_in, ŷ = (y-μ_out)/σ_out to the model during TRAIN/VAL.
#   - EXPORTS (TorchScript) take RAW inputs in physical units (ax,ay,az,wx,wy,wz,cpsi,spsi) and:
#       * normalize inputs inside TS,
#       * denormalize mean outputs to PHYSICAL units,
#       * scale variances to PHYSICAL units.
#   -> C++ always passes raw IMU + (cpsi,spsi) and receives physical μ and covariance.
#
# Architecture (fixed):
#   Input FC + ReLU: 8 → 120
#   GRU stack: 3 layers, hidden=120 (batch_first=True)
#   Dropout  : p=0.5 on the last GRU outputs
#   Heads    : fc_mu 120→7   (μ̂ = normalized means)
#              fc_lv 120→7   (log-variance in normalized space)
#
# Training:
...
#   one ensemble TorchScript (returns PHYSICAL μ_mean and FULL predictive covariance [7x7]).
#
# Example usage: 
# python3 scripts/nn_observer_v40.py --train data/nn_dataset_v4_X_C0/train.csv  --val data/nn_dataset_v4_X_C0/val.csv --out data/nn_model_v2_ens8_0 --seq 100 --epochs 1000 --gpu --ensemble 3 --bagging_frac 0.9 --switch_to_nll_ep 3200 --norm_json data/nn_dataset_v4_X_C0/norm_stats.json
# -------------------------------------------------------
...
# ---------------- I/O columns ---------------- #
# NOTE: The dataset must contain these exact columns for inputs and targets.
INPUT_COLS  = ["ax","ay","az","wx","wy","wz","cpsi","spsi"]   # 8D IMU + (cosψ,sinψ)
TARGET_COLS = ["Vn","Ve","p","q","r","phi","theta"]             # 7D outputs
...
# ---------------- Model: FC(8→120) + GRU×3(H=120) + Dropout(0.5) + Heads(120→7,120→7) ---------------- #

class GRUObserverV4(nn.Module):
    """
    ...
    Training path:
        x̂: [B,T,8] → FC: [B,T,120] → GRU×3: [B,T,120] → Dropout → (μ̂, logvar̂): [B,T,7] each
    """
    def __init__(self):
        super().__init__()
        self.in_dim: int = 8
        self.embed_dim: int = 120
        self.hidden: int = 120
        ...
        self.fc_mu  = nn.Linear(self.hidden, 7)
        self.fc_lv  = nn.Linear(self.hidden, 7)  # log(var) in normalized space
    ...
    def forward_seq(self, x_hat: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        x̂ [B,T,8] -> (μ̂, logvar̂) each [B,T,7]
        """
        if x_hat.dim() != 3 or x_hat.size(-1) != self.in_dim:
            raise ValueError(f"forward_seq expects [B,T,{self.in_dim}]")
        x_emb = self.input_fc(x_hat)               # [B,T,120]
        out, _ = self.gru(x_emb)                   # [B,T,120]
        mu_hat, logvar_hat = self._heads(out)      # [B,T,7]
    ...
    def __repr__(self) -> str:
        return (f"{self.__class__.__name__}(in_dim={self.in_dim}, embed_dim={self.embed_dim}, "
                ...
                f"HEADS='mu_hat:120→7, logvar_hat:120→7 (normalized)')")
...
def nll_diag(mu_hat: torch.Tensor, logvar_hat: torch.Tensor, y_hat: torch.Tensor) -> torch.Tensor:
    ...
    Diagonal Gaussian NLL in normalized space (ignoring constant 0.5*log(2π)):
      0.5 * [ logvar̂ + (ŷ - μ̂)^2 / exp(logvar̂) ], sum over D dims, mean over batch*time
    """
...
class MetricPlot:
    """
    Live plot for N-channel metrics (e.g., MAE or RMSE) over epochs.
...
class WindowDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        Ywin = self.Y_hat[start_idx:end_idx+1, :]   # [T,7] normalized
...
class _ExportStateless(nn.Module):
    ...
    def __init__(self, core: GRUObserverV4,
                 in_mu: torch.Tensor, in_std: torch.Tensor,
                 out_mu: torch.Tensor, out_std: torch.Tensor):
        super().__init__()
        self.core = core
        self.IN_DIM = 8
        # register buffers for stats (shape [1,8] for broadcast)
        self.register_buffer("in_mu",  in_mu.view(1, self.IN_DIM))
        self.register_buffer("in_std", in_std.view(1, self.IN_DIM))
        self.register_buffer("out_mu",  out_mu.view(1, 7))
        self.register_buffer("out_std", out_std.view(1, 7))
        self.register_buffer("log_out_std2", (2.0 * torch.log(out_std.view(1, 7))))
    ...
    def forward(self, x_raw: torch.Tensor, h_prev: Optional[torch.Tensor]):
        ...
        x_emb = self.core.input_fc(x_hat)                         # [B,120]
        out, h_next = self.core.gru(x_emb.unsqueeze(1), h_prev)   # [B,1,120], [L,B,120]
        z = self.core.dropout(out[:, -1, :])                      # [B,120]
        mu_hat = self.core.fc_mu(z)                               # [B,7] (normalized mean)
        logvar_hat = torch.clamp(self.core.fc_lv(z), -10.0, 5.0)  # [B,7] (normalized log-var)

        # denormalize outputs to PHYSICAL
        mu_phys = mu_hat * self.out_std + self.out_mu             # [B,7]
        logvar_phys = logvar_hat + self.log_out_std2              # [B,7]

        return mu_phys, logvar_phys, h_next


def export_torchscript_stateless(model: GRUObserverV4, out_path: Path, norm: NormStats):
...
class _EnsembleExportStateless(nn.Module):
    ...
    forward(x_raw, Hprev) -> (mu_phys_mean, cov_phys, Hnext)
      ...
      mu_phys_mean: [B,7]      (PHYSICAL mean)
      cov_phys:     [B,7,7]    (PHYSICAL predictive covariance — FULL matrix)
      Hnext:        [M,L,B,H]
    """
    __constants__ = ['M','L','H']
    ...
    def forward(self, x_raw: torch.Tensor, Hprev: Optional[torch.Tensor]):
        ...
        for m, member in enumerate(self.members):
            mu_phys_m, lv_phys_m, Hm_next = member(x_raw, Hprev[m])   # [B,7], [B,7], [L,B,H]
            mus.append(mu_phys_m)
            logvars.append(lv_phys_m)
            Hnext_list.append(Hm_next.unsqueeze(0))                   # [1,L,B,H]

        MU = torch.stack(mus, dim=0)                    # [M,B,7] physical means
        LV = torch.stack(logvars, dim=0)                # [M,B,7] physical log-variances
        VAR_m = torch.exp(LV)                           # [M,B,7] per-model variance (physical)

        mu_phys_mean = MU.mean(dim=0)                   # [B,7]

        # Aleatoric (diag) expected covariance E[Σ_m]
        e_sigma_diag = VAR_m.mean(dim=0)                # [B,7]
        cov_aleatoric = torch.diag_embed(e_sigma_diag)  # [B,7,7]

        # Epistemic covariance of means Cov_m[μ_m] (FULL matrix)
        centered = MU - mu_phys_mean.unsqueeze(0)       # [M,B,7]
        cov_epistemic = torch.einsum('mbi,mbj->bij', centered, centered) / float(M)  # [B,7,7]

        cov_phys = cov_aleatoric + cov_epistemic        # [B,7,7]
...
def main():
    ...
    ap.add_argument("--out", default="data/nn_model_v4", help="Output folder")
    ap.add_argument("--epochs", type=int, default=50)
    ap.add_argument("--batch", type=int, default=1024)
    ap.add_argument("--seq", type=int, default=50, help="GRU unroll/window length")
    ap.add_argument("--lr", type=float, default=1e-4) # Lower gives more stable training (but perhaps slower)
    ...
    for m in range(1, args.ensemble + 1):
        ...
        for ep in range(1, args.epochs + 1):
            ...
            if dl_val is not None:
                # evaluate must return (val_mse, val_nll, mse_ch) where mse_ch is per-output MSE in physical units
    ...
    with open(out_dir / "config.json", "w") as f:
        json.dump({
            "arch": {
                "in_dim": 8, "embed_dim": 120, "hidden": 120, "layers": 3, "dropout_p": 0.5,
                "heads": {"mu_hat": "120->7", "logvar_hat": "120->7 (normalized)"}
            },
            "seq": args.seq,
            "lr": args.lr,
            "epochs": args.epochs,
