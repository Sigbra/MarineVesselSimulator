...
std::vector<double> NLOptControlAlloc(double Fx_des, double Fy_des, double Mz_des) {
    ...
    double L1 = 1;  // Distance of left thruster from centerline
    double L2 = 1;  // Distance of right thruster from centerline
    ...
    MX objective = 0;
    if (!std::isnan(Fx_des)) {
        objective += pow(Fx - Fx_des, 2);
    }
    if (!std::isnan(Fy_des)) {
        objective += pow(Fy - Fy_des, 2);
    }
    if (!std::isnan(Mz_des)) {
        objective += 0.1 * pow(Mz - Mz_des, 2);
    }
