...
// void NN_qObs_Aided_EKF_v11::setRotationFromQuat(const Eigen::Quaterniond& q_in)
// {
//   // QuatObserver already outputs BODY→END.
//   Eigen::Quaterniond q = q_in; //.normalized();

//   //Deterministic canonicalization to avoid sign changes at 180°
//   if (q.w() < 0.0 || (std::abs(q.w()) <= 1e-12 && q.z() < 0.0)) {
//     q.coeffs() *= -1.0;
//   }
...
// #ifdef EKF_DEBUG
//   const double yaw = HeadingFromRnb_END(R_nb_);
//   dbgHeader("setQ");
//   dbgPrintQuat("q_can", q); std::cerr << "  yaw="<< rad2deg(yaw) << " deg\n";
...
//   // 1) Normalize and ensure BODY→NAV
//   Eigen::Quaterniond q = q_nb; //.normalized();

//   // 2) Keep NN input quaternion continuous across calls (avoid ±q flip)
//   static Eigen::Quaterniond q_prev_nn(1.0, 0.0, 0.0, 0.0);
//   static bool has_prev_nn = false;
//   if (has_prev_nn) {
//     const double dot = q.w()*q_prev_nn.w() + q.x()*q_prev_nn.x()
//                      + q.y()*q_prev_nn.y() + q.z()*q_prev_nn.z();
//     if (dot < 0.0) {
//       q.coeffs() *= -1.0;
//     }
//   }
...
//   // 3) Buffer one-step NN input row
//   Eigen::Matrix<double,10,1> row;
//   row << accel_b.x(), accel_b.y(), accel_b.z(),
//          q.w(), q.x(), q.y(), q.z(),
//          tau_x, tau_y, tau_n;
