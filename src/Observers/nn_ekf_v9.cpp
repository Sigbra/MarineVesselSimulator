// nn_ekf_v9.cpp — 9-state EKF (“NN_qObs_Aided_EKF_v9”) with external attitude (q-Obs)
// + Velocity-only NN_v9 TorchScript wrapper (PIMPL) for END-frame velocity aiding.
// v9 difference from v7: inputs to NN are 10 scalars:
//   [ax, ay, az, qw, qx, qy, qz, tau_x, tau_y, tau_n]
// Accel and tau are plain scalars (NO quaternion packing). Only the quaternion is a quaternion.
            ...
            bool use_cuda)
  {
    ...
    YAML::Node ns;
    {
      ...
      try {
        ...
        std::cerr << "[NN_v9] Failed to open norm_stats: " << norm_json << "\n";
        return false;
      }
    }
    ...
    if (!x_mean || !x_std || !y_mean || !y_std) {
      std::cerr << "[NN_v9] norm_json missing keys x_mean/x_std/y_mean/y_std\n";
      return false;
    }
...
#endif
    if (!fs::exists(md)) {
      std::cerr << "[NN_v9] Model path does not exist: " << md << "\n";
      return false;
    }
    ...
    if (files.empty()) {
      std::cerr << "[NN_v9] No TorchScript files found at: " << md << "\n";
      return false;
    }
    ...
    try {
      ...
      for (const auto& f : files) {
        ...
        std::cerr << "[NN_v9] Loaded " << f << (use_cuda_ ? " (CUDA)\n" : " (CPU)\n");
      }
    } catch (const c10::Error& e) {
      std::cerr << "[NN_v9] Failed to load model: " << e.what() << "\n";
      return false;
    }

    if (members_.size() == 1) {
      std::cerr << "[NN_v9] WARNING: only 1 model loaded; covariance will be near floor.\n";
    } else {
      std::cerr << "[NN_v9] Ensemble size: " << members_.size() << "\n";
    }
    return true;
  }
             ...
             Mat3& Rv_nav)
  {
    ...
    for (int m=0; m<M; ++m) {
      ...
      if (y.dim()!=3 || y.size(0)!=1 || y.size(2)!=3) {
        std::cerr << "[NN_v9] Bad output shape from member " << m << ": " << y.sizes() << "\n";
        return false;
      }
...
NN_v9::NN_v9()  : impl_(std::make_unique<Impl>()) {}
NN_v9::~NN_v9() = default;

bool NN_v9::init(const std::string& model_dir,
                 const std::string& norm_json,
                 bool use_cuda) {
  return impl_->init(model_dir, norm_json, use_cuda);
}

bool NN_v9::infer(const std::vector<Eigen::Matrix<double,10,1>>& window,
                  Vec3& v_nav_mean,
                  Mat3& Rv_nav) {
...
NN_qObs_Aided_EKF_v9::NN_qObs_Aided_EKF_v9(const Config_v9& cfg) : cfg_(cfg) {
  P_.setIdentity();
...
void NN_qObs_Aided_EKF_v9::setRotationNavFromBody(const Mat3& R_nb) { R_nb_ = R_nb; }

void NN_qObs_Aided_EKF_v9::setRotationFromQuat(const Eigen::Quaterniond& q_nb) {
  R_nb_ = RnbFromQuatCustom(q_nb);  // custom END convention
}

void NN_qObs_Aided_EKF_v9::setNN(NN_v9* nn, int seq_len, int stride) {
  nn_ = nn;
...
void NN_qObs_Aided_EKF_v9::nn_prune_() {
  const int MAX_KEEP = std::max(nn_seq_len_, 4);
  while ((int)nn_buf_.size() > MAX_KEEP) nn_buf_.pop_front();
}

// Feed one sample to NN buffer; accel and tau are scalars, quaternion is normalized.
void NN_qObs_Aided_EKF_v9::feedNN(const Vec3& accel_b,
                                  ...
                                  double tau_n)
{
  ...
  // 1) Buffer the latest input row [ax, ay, az, qw, qx, qy, qz, tau_x, tau_y, tau_n]
  ...
  Vec3 v_nav_mean;   // as produced by NN (END: [vE,vN,vD])
  Mat3 Rv_nav;
  if (!nn_->infer(window, v_nav_mean, Rv_nav)) return;

  // 4) Apply the EKF velocity update in END
  (void)updateNNVelNav(v_nav_mean, Rv_nav, /*w=*/1.0);
}

bool NN_qObs_Aided_EKF_v9::updateNNVelNav(const Vec3& z_v_nav, const Mat3& Rv, double w) {
  ...
  // Optional NIS & gate (kept like v7; disabled if chi2_gate_vec3<=0)
...
const State9_v9& NN_qObs_Aided_EKF_v9::state() const { return x_; }
const Mat99&     NN_qObs_Aided_EKF_v9::cov()   const { return P_; }

void NN_qObs_Aided_EKF_v9::setState(const State9_v9& x, const Mat99& P){ x_ = x; P_ = P; }

void NN_qObs_Aided_EKF_v9::propagate(const Vec3& omega_b_meas, const Vec3& accel_b_meas, double dt) {
...
bool NN_qObs_Aided_EKF_v9::updateGnssPos(const Vec3& z_nav_ant, const Mat3& Rpos, const Vec3& r_body, double w){
...
bool NN_qObs_Aided_EKF_v9::updateGnssVel(const Vec3& z_v_nav_ant, const Mat3& Rvel, const Vec3& r_body,
...
bool NN_qObs_Aided_EKF_v9::updateNNBodyVel(const Vec3& z_v_b, const Mat3& Rvb, double w){
...
Eigen::Matrix<double,9,1> NN_qObs_Aided_EKF_v9::getState9() const {
...
Eigen::VectorXd NN_qObs_Aided_EKF_v9::getState12(const Vec3& b_gyro_hat) const {
...
bool NN_qObs_Aided_EKF_v9::kalmanUpdate(const Vec3& r, const Eigen::Matrix<double,3,9>& H,
