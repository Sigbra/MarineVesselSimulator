...
// Wrap angle to [-pi, +pi) (radians). Matches Python: (a+pi)%(2pi)-pi.
static inline double wrap_pm_pi(double a){
  double y = std::fmod(a + M_PI, 2.0 * M_PI);
  if (y < 0.0) y += 2.0 * M_PI;
  return y - M_PI;
}
                                  ...
                                  double tau_n)
{
  ...
  // -------------------------------------------------------------
  // Use Utilities' END-consistent attitude construction/extraction
  // -------------------------------------------------------------
  const Mat3 R = RnbFromQuatCustom(q_nb);   // BODY -> NAV (END)

  auto clamp1 = [](double s){ return std::max(-1.0, std::min(1.0, s)); };

  // Inverse of your RnbFromEuler() (END, ZYX):
  // R(2,0) = -sin(theta)
  ...
  // Wrap using Utilities. If you require [-pi, +pi) exactly, map +pi -> -pi.
  auto wrap_pm_pi_nn = [](double a){
    a = ssa(a);                  // (-pi, pi]
    if (a >= M_PI) a -= 2*M_PI;  // -> [-pi, pi)
    return a;
  };

  phi   = wrap_pm_pi_nn(phi);
  theta = wrap_pm_pi_nn(theta);
  psi   = wrap_pm_pi_nn(psi);
  ...
  // Warm-start guard: do NOT call infer() before nn_seq_len_ samples exist.
  if ((int)nn_buf_.size() < nn_seq_len_) {
    nn_warmed_ = false;
    return;
  }
  ...
  if (!nn_warmed_) {
    ...
    // Only apply EKF update on stride (but warm-start state is filled regardless).
    if ((nn_count_ % nn_stride_) == 0) {
      (void)updateNNVelNav(v_nav_mean, Rv_nav, /*w=*/1);
    }
    return; // avoid double-feeding the last sample this tick
