...
// with only necessary changes for Euler-only inputs and robust hidden-state init
// even when TorchScript exports do not provide init_state().

#include "Observers/nn_ekf_v12.hpp"
#include "Utilities/calculations.hpp"   // Quat, ssa(), RnbFromQuatCustom(...)
                                  ...
                                  double tau_n)
{
  ...
  if (!nn_warmed_) {
    ...
    if ((nn_count_ % nn_stride_) == 0) {
      (void)updateNNVelNav(v_nav_mean, Rv_nav, /*w=*/0.1);
    }
    return;
  }
...
// //  - All angles (phi/theta/psi) are wrapped to [-pi, +pi) (±180°) before normalization.
// //
// // This implementation is a drop-in replacement that does NOT require header edits.
...
//   // ---- model input dimension (from norm_stats.json) ----
//   // v12 setup: D MUST be 9 (Euler-only)
//   int in_dim_ = 0;
...
//     if (files_stateful.empty()) {
//       std::cerr << "[NN_v12] No stateful members found (files named member_*_onestep_stateful.pt) in: "
//                 << md << "\n";
//       return false;
//     }
...
//   //       } catch (const c10::Error& e) {
//   //         std::cerr << "[NN_v12] init_state() missing on member " << m
//   //                   << " (" << e.what() << ")\n"
...
//   //     // Step through the required part of the window to update h_[m]
//   //     for (int i = start_idx; i < T; ++i) {
//   //       const auto& r = window[i];

//   //       const double ax = r(0), ay = r(1), az = r(2);

//   //       // Wrap Euler to match dataset/training ([-pi, +pi))
//   //       const double phi   = wrap_pm_pi(r(3));
//   //       const double theta = wrap_pm_pi(r(4));
//   //       const double psi   = wrap_pm_pi(r(5));

//   //       const double tau_x = r(6), tau_y = r(7), tau_n = r(8);

//   //       // Fill x (D=9 Euler-only):
...
//   //       torch::Tensor x_norm =
//   //         (x - x_mean_t_.view({1,1,in_dim_})) / x_std_t_.view({1,1,in_dim_});
//   //       torch::Tensor xf = x_norm.to(torch::kFloat); // TorchScript traced with float inputs

//   //       // Forward: forward(x:[1,1,D], h:[L,1,H]) -> (y:[1,1,3], h_next)
//   //       c10::IValue out_iv;
//   //       try {
//   //         out_iv = members_[m].forward({ xf, h_[m] });
//   //       } catch (const c10::Error& e) {
...
//   //       y_last = elems[0].toTensor();            // [1,1,3]
//   //       torch::Tensor hnext = elems[1].toTensor(); // [L,1,H]
//   //       h_[m] = hnext;                           // persist hidden at every stepped sample
//   //     }
...
//       Vec3 v;
//       for (int k = 0; k < 3; ++k) v(k) = vv[k]*y_std_[k] + y_mean_[k];
//       preds.push_back(v);
//     }
...
// void NN_v12::reset_states() {
//   // simplest: clear hidden state cache; it will re-init lazily on next infer()
//   // (per-member init_state(1) is called inside infer when h_ is empty)
//   if (impl_) {
//     // If you want an explicit reset without reconstructing the EKF, add a reset method to Impl
//     // that clears h_. For now, the EKF-level lifecycle typically handles this.
//   }
// }
...
//   // Ensure the NN hidden state is cleared so next infer(window) truly warm-starts
//   if (nn_) nn_->reset_states();
// }
...
// // Feed one sample to NN buffer.
// // v12 Euler NN uses EKF attitude (R_nb_) to derive phi/theta/psi.
// // Signature kept for API compatibility; q_nb is not used by the NN feature build.
// // void NN_qObs_Aided_EKF_v12::feedNN(const Vec3& accel_b,
// //                                   const Quat& q_nb,
// //                                   double tau_x,
// //                                   double tau_y,
...
// //   // Euler extraction from END-convention R_nb_ (same as getState12()).
// //   auto clamp1 = [](double s){ return std::max(-1.0, std::min(1.0, s)); };
// //   const double r20 = R_nb_(2,0);
// //   const double r21 = R_nb_(2,1);
// //   const double r22 = R_nb_(2,2);
...
// //   double theta = -std::asin(clamp1(r20));  // pitch
// //   double phi   = std::atan2(r21, r22);     // roll
// //   double psi   = std::atan2(r00, r10);     // heading from North→East (END convention)

// //   // v12 Euler-only training uses wrapped angles ([-pi, +pi)); do NOT unwrap.
// //   phi   = wrap_pm_pi(phi);
...
// //   // the GRU hidden state gets initialized on too-short history and we lose the point of warm-starting (for first call).
// //   if ((int)nn_buf_.size() < nn_seq_len_) {
// //     nn_warmed_ = false;
// //     return;
// //   }
...
// // }
// // Feed one sample to NN buffer.
// // v12 Euler NN uses EKF attitude to derive phi/theta/psi in END convention.
...
//   // Defensive canonicalization (±q doesn't change R, but avoids degenerate norms)
//   const Quat q_can = canonicalizeQuat(q_nb);
//   const Mat3 R = RnbFromQuatCustom(q_can);   // BODY -> NAV (END)

//   auto clamp1 = [](double s){ return std::max(-1.0, std::min(1.0, s)); };

//   // Inverse of END ZYX Euler (consistent with your conventions)
//   double theta = std::asin(clamp1(-R(2,0)));   // pitch
//   double phi   = std::atan2(R(2,1), R(2,2));   // roll
//   double psi   = HeadingFromRnb_END(R);        // heading (North -> East)
...
// //   double theta = -std::asin(clamp1(r20));  // pitch
// //   double phi   = std::atan2(r21, r22);     // roll
// //   double psi   = std::atan2(r00, r10);     // heading from North→East

// //   // Keep reported angles consistent with v12 convention
// //   phi   = wrap_pm_pi(phi);
// //   theta = wrap_pm_pi(theta);
// //   psi   = wrap_pm_pi(psi);
...
//   // Keep reported angles consistent with v12 convention: [-pi, +pi)
//   phi   = wrap_pm_pi_ssa(phi);
//   theta = wrap_pm_pi_ssa(theta);
//   psi   = wrap_pm_pi_ssa(psi);
